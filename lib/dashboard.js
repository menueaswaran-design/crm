...
export async function getDashboardSummary(user) {
  ...
  const clientFilter = isAdmin ? { ...scope } : { assignedStaff: user._id, ...scope };
  const invoiceFilter = isAdmin ? { ...scope } : clientIds ? { clientId: { $in: clientIds } } : {};
  const paymentFilter = isAdmin ? { ...scope } : clientIds ? { clientId: { $in: clientIds } } : {};

  const [totalClients, complianceBuckets, taskBuckets, invoiceSums, paymentSums, documents, expenseSums] =
    await Promise.all([
      ...
      Document.countDocuments({ isDeleted: { $ne: true }, ...scope }),
      Expense.aggregate([
        { $match: { ...scope } },
        { $group: { _id: null, total: { $sum: "$amount" } } },
      ]),
  ...
  const totalExpenses = expenseSums[0]?.total || 0;
  ...
  return {
    ...
    tasks: tasksByStatus,
    totalRevenue: viewFinancials ? totalRevenue : 0,
    amountReceived: viewFinancials ? amountReceived : 0,
    outstandingAmount: viewFinancials ? outstandingAmount : 0,
    totalExpenses: viewFinancials ? totalExpenses : 0,
    documents,
  };
}
...
export async function getRevenueSeries(user) {
  ...
  if (!canViewFinancials(user)) return [];

  const scope = companyScope(user) || {};
  const clientIds = await staffClientIds(user);
  const filter = clientIds ? { clientId: { $in: clientIds } } : scope;
...
export async function getUpcomingDeadlines(user) {
  ...
  const scope = companyScope(user) || {};
  const today = new Date();
  const staffQuery = isAdmin ? { ...scope } : { assignedStaff: user._id, ...scope };
