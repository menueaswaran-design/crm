...
export const NAV_PERMISSIONS = [
  ...
  { key: "invoices", label: "Invoices", href: "/invoices", description: "Billing & payments" },
  {
    ...
    label: "AI Analyzer",
    href: "/ai-analyzer",
    description: "Optional: extract notice details with Gemini (pro add-on)",
  },
];
...
export function getDefaultRoute(user) {
  ...
  if (user.role === "superAdmin") return SUPER_ADMIN_ROUTE;
  if (user.role === "admin" && hasPermission(user, "dashboard")) return "/today";
...
export function canAccessPath(user, pathname) {
  ...
  if (segment === "/super-admin") return user.role === "superAdmin";
