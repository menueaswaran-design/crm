...
/**
 * Verifies a Firebase ID token via Google JWKS endpoint.
 */
export async function verifyFirebaseIdToken(token) {
  const projectId = "crm-ca-db172";
...
/**
 * Verifies the user AND that they have access to the given module.
 * Admins always pass; staff need the module key in their permissions.
 */
export async function requirePermission(request, moduleKey) {
  const user = await requireAuth(request);
  if (!hasPermission(user, moduleKey)) {
    throw new AuthError(403, "You do not have access to this section.");
  }
  return user;
}

/**
 * Tenant scope for a company admin / staff user.
 * Super admin (no company) gets null so they can never reach tenant data.
 */
...
/**
 * Verifies the user is the super admin. Throws on failure.
 */
