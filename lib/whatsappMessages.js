 ...
 */
export function generateDocumentRequestMessage({ client, documents, period, filingType }) {
  const name = client?.name || "Client";
  const periodLabel = period || "the current period";
  const filing = filingType ? ` (${filingType})` : "";
  const list = documents || [];
  const docList = list.length
    ? list.map((d, i) => `${i + 1}. ${d.name || d}`).join("\n")
    : "1. Required supporting documents";
  const noun = list.length === 1 ? "document" : "documents";

  return `Hello ${name},

We are preparing your compliance work for ${periodLabel}${filing}.

Please share the following ${noun}:
