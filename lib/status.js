 ...
 * Fully tenant-scoped: each company's rows are updated in isolation.
 *
  ...
  inflightOverdue = (async () => {
    ...
    for (const companyId of companyIds) {
      await Compliance.updateMany(
        { companyId, status: { $ne: "COMPLETED" }, dueDate: { $lt: today } },
        { $set: { status: "OVERDUE" } }
      );
    }
  })()
...
/** MongoDB filter for task list queries using derived status rules. */
export function buildTaskStatusFilter(status) {
  ...
  if (status === "OVERDUE") {
    return { status: { $ne: "COMPLETED" }, dueDate: { $lt: now } };
  }
  if (status === "PENDING" || status === "IN_PROGRESS") {
    return {
      status,
      $or: [{ dueDate: { $gte: now } }, { dueDate: null }, { dueDate: { $exists: false } }],
    };
  }
